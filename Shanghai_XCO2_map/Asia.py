...
m = Basemap(projection='cyl', resolution='f', llcrnrlat=26, urcrnrlat=35,llcrnrlon=115, urcrnrlon=125,ax=ax1)
...
for info,shp in zip(m.states_info,m.states):
    proid = info['NAME_1']
    if proid =='Shanghai':
        poly = Polygon(shp,facecolor='grey',lw=1,alpha=1)
        ax1.add_patch(poly)
...
m.drawmeridians(np.arange(115, 125, 5), labels=[1,0,0,1])
m.drawparallels(np.arange(25, 35.001, 5), labels=[1,0,0,1])
m.scatter(longitude_gosat, latitude_gosat,c='r',zorder=10, s=8,label='GOSAT')
m.scatter(longitude_oco, latitude_oco,c='b',zorder=10, s=0.5,label='OCO-2')
...
#m.drawmapscale(130, 25, 133, 30, 100, barstyle='fancy')
